...
ranges. Identical to the legacy v2 EDGEPROXY stack — preserves
byte-for-byte network layout compatibility with apps already attached
to those networks.
...
**The CGNAT choice is deliberate:** BG hosts typically give `docker0`
a large RFC1918 range (often `10.0.0.0/9`) for general-purpose stack
pools. Putting the proxy networks in CGNAT keeps Traefik traffic in a
separate, recognizable address family that does not collide with the
stack pool — visual + topological separation, not arbitrary.
...
## Subnet collision troubleshooting

If `traefik.sh start` fails with `Pool overlaps with other one on
this address space`, the host already has something in one of the
v2-compat subnets (CGNAT v4 / `fdff:100:6{4,5}::` v6).

**Diagnose first:**

```bash
# Host-level routes / interfaces in our ranges?
ip -4 addr show | grep -E 'inet 100\.6[45]\.'
ip -6 addr show | grep -E 'inet6 fdff:'
ip route show | grep -E '^(100\.6[45]\.|fdff:)'

# Docker daemon config (default-address-pools / fixed-cidr-v6)?
cat /etc/docker/daemon.json

# What does docker0 actually claim?
...
- **Tailscale** — claims all of `100.64.0.0/10` at the kernel
  routing level. Either remove Tailscale, or hand-edit
  `docker-compose.yml` networks block to a non-CGNAT IPv4 range
  for that host.
- **`fixed-cidr-v6: fdff::/17`** in `daemon.json` — claims the
  lower half of `fdff::/16` for `docker0`. The `fdff:100:64::/64`
  / `fdff:100:65::/64` defaults fall inside that. Either change
  the daemon's IPv6 CIDR, or hand-edit the IPv6 prefix in the
  Compose `networks:` block to something OUTSIDE `fdff::/17`
  (e.g. `fdff:8064::/64` — the `8` flips bit 17 into the upper
  half of `fdff::/16`).
- **`default-address-pools`** in `daemon.json` claiming the CGNAT
  range for general-purpose Docker bridges.

The subnets are intentionally hard-coded in `docker-compose.yml`
(no env-var indirection) to preserve byte-for-byte v2 compatibility
for app stacks. Treat a collision as a host-config issue first; if
host config genuinely cannot change, hand-edit the Compose
`networks:` block on that host (and the matching `dashboard-internal`
ClientIP rule on the Traefik service).
